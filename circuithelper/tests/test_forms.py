...
class TestCircuitCostForm:
    ...
    def test_form_with_missing_circuit(self):
        ...
        form_data = {"mrc": Decimal("150.00"), "currency": "USD"}
        form = CircuitCostForm(data=form_data)
    ...
    def test_form_with_optional_fields(self, circuit):
        ...
        form_data = {"circuit": circuit.pk, "currency": "USD"}
        form = CircuitCostForm(data=form_data)
        assert form.is_valid()

    def test_form_decimal_validation(self, circuit):
        """Test decimal field validation."""
        form_data = {"circuit": circuit.pk, "mrc": "invalid", "currency": "USD"}
        form = CircuitCostForm(data=form_data)
        assert not form.is_valid()
...
class TestCircuitContractForm:
    ...
    def test_form_with_file_upload(self, circuit):
        ...
        form = CircuitContractForm(data=form_data, files={"contract_file": uploaded_file})
        assert form.is_valid()
...
class TestCircuitPathForm:
    ...
    def test_form_with_kmz_upload(self, circuit):
        ...
        uploaded_file = SimpleUploadedFile(
            "path.kmz", file_content, content_type="application/vnd.google-earth.kmz"
        )

        form_data = {"circuit": circuit.pk, "map_zoom": 10}
        form = CircuitPathForm(data=form_data, files={"kmz_file": uploaded_file})
        assert form.is_valid()
