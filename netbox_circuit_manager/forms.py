...
class CircuitCostForm(NetBoxModelForm):
    ...
    class Meta:
        model = CircuitCost
        fields = [
            'circuit', 'nrc', 'mrc', 'currency',
            'billing_account', 'last_updated_date',
            'tags', 'comments'
        ]
...
class CircuitContractForm(NetBoxModelForm):
    ...
    class Meta:
        ...
        fields = [
            'circuit', 'contract_number', 'start_date', 'end_date',
            'term_months', 'auto_renew', 'renewal_notice_days',
            'early_termination_fee', 'contract_file', 'notes',
            'tags', 'comments'
        ]
        widgets = {
...
class CircuitTicketForm(NetBoxModelForm):
    ...
    class Meta:
        model = CircuitTicket
        fields = [
            'circuit', 'ticket_number', 'subject', 'status',
            'priority', 'closed_date', 'description',
            'resolution', 'external_url', 'tags', 'comments'
        ]
        widgets = {
...
class CircuitPathForm(NetBoxModelForm):
    ...
    class Meta:
        model = CircuitPath
        fields = [
            'circuit', 'kmz_file', 'map_center_lat',
            'map_center_lon', 'map_zoom', 'path_distance_km',
            'path_notes', 'tags', 'comments'
        ]
        widgets = {
...
class ProviderAPIConfigForm(NetBoxModelForm):
    class Meta:
        ...
        fields = [
            'provider', 'provider_type', 'api_endpoint',
            'api_key', 'api_secret', 'sync_enabled',
            'sync_interval_hours', 'tags', 'comments'
        ]
        widgets = {
